...
class IO():
    ...
    def load_weights(self, model, weights_path, ignore_weights=None):
        ...
        for w, v in weights.items():
            if not model.ensemble:
                if 'edge_importance.ntu' in w or 'A_ntu' in w:
                    v = v[:, :25, :25]  # Slice the weights to the desired shape
                    weights[w] = v
    ...
    def save_model(self, model, name, optimizer=None, epoch=None):
        model_path = '{}/{}'.format(self.work_dir, name)
        checkpoint_dir = '{}/{}'.format(self.work_dir, 'checkpoints')
        checkpoint_path = '{}/{}/{}'.format(self.work_dir, 'checkpoints', name)

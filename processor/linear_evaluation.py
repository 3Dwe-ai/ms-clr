...
class LE_Processor(Processor):
    ...
    def load_model(self):
        ...
        if 'skeletons' in self.arg.model_args:
            num_skeletons = len(self.arg.model_args['skeletons'])
        else:
            num_skeletons = 1

        for name, param in self.model.encoder_q.named_parameters():
            if not (name.startswith('fc.') and (name.endswith('.weight') or name.endswith('.bias'))):
                param.requires_grad = False
        self.num_grad_layers = 2*num_skeletons
        ...
        for data, label in loader:
            ...
            if self.model.ensemble:
                for skeleton in data:
                    data[skeleton] = data[skeleton].float().to(self.dev, non_blocking=True)
            else:
            ...
            if self.arg.stream == 'joint':
                ...
                if self.model.ensemble:
                    for skeleton in data: 
                        motion = torch.zeros_like(data[skeleton])
                        motion[:, :, :-1, :, :] = data[skeleton][:, :, 1:, :, :] - data[skeleton][:, :, :-1, :, :]
                        data[skeleton] = motion
                else:
                ...
                if self.model.ensemble:
                    for skeleton in data:
                        bone = torch.zeros_like(data[skeleton])
                        for v1, v2 in Bone:
                            bone[:, :, :, v1 - 1, :] = data[skeleton][:, :, :, v1 - 1, :] - data[skeleton][:, :, :, v2 - 1, :]
                        data[skeleton] = bone
                else:
            ...
            if self.model.ensemble:
                ...
                for skeleton in data:
                    output = self.model(None, data[skeleton], q_skeleton=skeleton)
                    losses.append(self.loss(output, label))
                loss = sum(losses)
            else:
    ...
    def test(self, epoch):
        ...
        for data, label in loader:
            ...
            with torch.no_grad():
                if self.model.ensemble:
                    output = torch.zeros((label.shape[0], self.arg.model_args['num_class']), device=self.dev)
                    for skeleton in data:
                        output += self.model(None, data[skeleton], q_skeleton=skeleton)
                    output /= len(data) # Takes the average of all classifier logits as output -> Soft voting
                else:
                    output = self.model(None, data)# This needs fixing
        ...
        self.wandb.log({'eval/loss':self.eval_info['eval_mean_loss'], 'epoch': epoch})
        self.show_eval_info()

        # show top-k accuracy 
        for k in self.arg.show_topk:
            self.show_topk(k)
            _, accuracy = self.get_topk(k)
            self.wandb.log({f'eval/Top{k}':accuracy, 'epoch': epoch})
            self.show_best(1)

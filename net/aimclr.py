...
class AimCLR(nn.Module):
    def __init__(self, skeletons=None, base_encoder=None, pretrain=True, ensemble=False, plot_interval=10, embeddings_per_batch=50, feature_dim=128, queue_size=32768,
        ...
        if not self.pretrain:
            self.encoder_q = base_encoder(skeletons=skeletons, in_channels=in_channels, hidden_channels=hidden_channels,
                                          ...
                                          edge_importance_weighting=edge_importance_weighting, ensemble=ensemble,
                                          **kwargs)
            if self.ensemble:
                self.encoder_q.fc = nn.ModuleDict()
                for skeleton in skeletons:
                    self.encoder_q.fc[skeleton] = nn.Linear(hidden_dim, num_class)
        else:
            self.K = queue_size
            self.m = momentum
            self.T = Temperature
    ...
    def forward(self, im_q_extreme, im_q, im_k=None, nnm=False, topk=1, q_skeleton='ntu-rgb+d', k_skeleton='ntu-rgb+d'):
        ...
        if not self.pretrain:
            return self.encoder_q(im_q, q_skeleton)

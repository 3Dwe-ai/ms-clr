...
class PersonDetector(torch.nn.Module):
    ...
    def forward(self, images, threshold, nms_iou_threshold, max_detections):
        # print(images.shape)
        # TODO Resize in the dataset and remove this
        h = np.float32(images.shape[2]) # Was images.shape[2]
        w = np.float32(images.shape[3]) # Was images.shape[3]
        ...
        # Issue when resizing: resize() expects dimensions ...H,W but batch is given as N,H,W,C
        # images = images.permute(0,3,1,2)
        images = torchvision.transforms.functional.resize(
             images, (int(target_h), int(target_w)), antialias=factor < 1)
        # print(images.shape)
        images = images ** (1 / 2.2)
        pad_h = -target_h % 32
        pad_w = -target_w % 32
